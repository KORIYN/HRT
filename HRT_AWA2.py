...
from core.helper_func import eval_zs_gzsl,visualize_attention,eval_zs_gzsl_loss#,get_attribute_attention_stats
...
dataloader = AWA2DataLoader(NFS_path)
...
elif mode == 'MultiStep28':
    optimizer = optim.RMSprop( params_to_update ,lr=lr,weight_decay=weight_decay, momentum=momentum)
    scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer,
                        milestones=[ 28], gamma=0.1)
...
number = 'AWA2seen-0.8unseen+1'

#visual_path
visual_path = pwd+'/visual/' + '{}{}bz={}lr=0.00001epoch=3000/'.format(number,mode,batch_size)

#store_path
save_dir = pwd+'/result/{}{}bz={},lr=0.00001,epoch=3000.txt'.format(number,mode,batch_size)
path = pwd +'/weight/{}{}bz={},lr=0.00001epoch=3000/'.format(number,mode,batch_size)
...
for i in range(0,niters):
    ...
    running_loss += loss.data.cpu()*batchsize
    running_loss_CE += loss_CE.data.cpu()*batchsize
    running_loss_cal += loss_cal.data.cpu()*batchsize
