...
class CapsNet(nn.Module):
    ...
    1. PrimaryCaps
        (_, 2048, 14, 14) -> 1x1 filter, 128 out capsules, stride 1, no padding
        x -> pose: (_, 14, 14, 128x4x4), activation: (_, 14, 14, 128)
    2. ConvCaps1
        (_, 14, 14, 128x(4x4+1)) -> 1x1 filters, 1 out capsules, stride 1, no padding
        x -> pose: (_, 14, 14, 1x4x4), activation: (_, 14, 14, 1)
    ...
    Args:
        ...
        ...
    """
    def __init__(self,A=2048, B=128, C=1, K=1, P=4, iters=3):
    ...
    def forward(self, x):
        ...
        shape = x.shape
        sqrt_shape2 = int(np.sqrt(shape[2]))
        x = x.reshape(shape[0],shape[1],sqrt_shape2,sqrt_shape2)
        x = self.primary_caps(x)
        _,x = self.conv_caps1(x)
        return x
